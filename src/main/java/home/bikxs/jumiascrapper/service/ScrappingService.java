    ...
    public class FileUpdater extends Thread implements Runnable {
        ...
        public void run() {
            ...
            while (!finished) {
                ...
                System.out.println(executor.getCompletedTaskCount() + "/" + executor.getTaskCount() + " tasks completed.");
                if (items.isEmpty()) {

                    System.out.println("Execution Queue");
                    for (Runnable runnable :
                            executor.getQueue()) {
                        SubCategoryScrapperThread scst = (SubCategoryScrapperThread) runnable;
                        System.out.println("\t" + scst.getSubCategory().toString());
                    }
                }
                ;
